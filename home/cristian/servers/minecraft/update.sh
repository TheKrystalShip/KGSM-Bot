...
download_version() {
  # Minecraft specific
  # local INSTALLED_VERSION="${WORKING_DIR}/installed_version"
  local MC_VERSIONS_CACHE="${WORKING_DIR}/_version_cache.json"
  local RELEASE_JSON="${WORKING_DIR}/_release.json"
  ...
  # Search version cache for latest
  ...
  LOCAL_RELEASE_JAR="$TEMP_DOWNLOAD_FOLDER/minecraft_server.$LATEST_RELEASE.jar"
  ...
  # Cleanup
  rm "$MC_VERSIONS_CACHE" "$RELEASE_JSON"
...
get_backup_folder() {
  ...
  mv_command_exit_code=$(exec cp "$source" "$destination_folder")
...
deploy_new_version() {
  ...
  mv_deploy_version_exit_code=$(mv -f "$TEMP_DOWNLOAD_FOLDER/*.jar" "$LATEST_INSTALLED_DIR/release.jar")
